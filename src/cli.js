...
function parseUserInput(input) {
  ...
  if (inputLower.includes("referencj") || inputLower.includes("reference")) {
    ...
    inputLower.includes("certyfikat") ||
    inputLower.includes("cert") ||
    inputLower.includes("zaświadczenie")
  ) {
    task = TaskType.CERT;
  } else if (
    inputLower.includes("staż") ||
    inputLower.includes("staz") ||
    inputLower.includes("internship") ||
    inputLower.includes("praktyk")
  ) {
...
function displayHelp() {
  ...
  console.log(
    chalk.bold("  certyfikat") + "   - Utwórz certyfikat/zaświadczenie"
  );
  console.log(
    chalk.bold("  staż") + "         - Utwórz dokument oceny stażu/praktyk"
  );
  ...
  console.log(
    '  "Utwórz certyfikat dla Anna Kowalska. Info: aktywny członek zespołu"'
  );
  console.log(
    '  "Wygeneruj referencje dla Jan Nowak. Opis: świetny programista, prowadził projekt API"'
  );
  console.log(
    '  "Dokument stażu dla Maria Wiśniewska z oceną wymagań uczelni"\n'
  );
}
...
async function manageTemplates() {
  ...
  const templateTypes = [
    {
      key: "references",
      name: "Referencje",
      file: config.templates.references,
    },
    { key: "cert", name: "Certyfikat", file: config.templates.cert },
    {
      key: "internship",
      name: "Staż/Praktyki",
      file: config.templates.internship,
    },
  ];
  ...
  for (const tmpl of templateTypes) {
    ...
    const status = exists
      ? chalk.green("✓ dostępny")
      : chalk.yellow("✗ brak pliku");
  ...
  let availableFiles = [];
  try {
    availableFiles = readdirSync(config.templatesDir).filter((f) =>
      f.endsWith(".pdf")
    );
  } catch {
    ...
    const { filePath } = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (!input.trim().toLowerCase().endsWith(".pdf"))
            return "Plik musi być w formacie PDF";
          return true;
        },
      },
    ]);
...
async function chatLoop() {
  ...
  console.log(
    chalk.blue.bold("║     Generator Dokumentów NGO                       ║")
  );
  console.log(
    chalk.blue.bold("║     Tworzenie dokumentów z użyciem AI              ║")
  );
  console.log(
    chalk.blue.bold("╚════════════════════════════════════════════════════╝")
  );
  ...
  while (true) {
    ...
    if (
      ["wyjście", "wyjscie", "quit", "exit", "q", "koniec"].includes(lowerInput)
    ) {
    ...
    if (!taskInput) {
      ...
      console.log(
        chalk.gray("   - Typ dokumentu: referencje, certyfikat, lub staż")
      );
    ...
    if (taskInput.task === TaskType.REFERENCES) {
      if (!taskInput.additionalInfo || taskInput.additionalInfo.trim() === "") {
        ...
        console.log(
          chalk.gray(
            "   Podaj informacje o: projektach, zaangażowaniu w onboarding, osiągnięciach, cechach charakteru.\n"
          )
        );
        ...
        const { additionalInfo } = await inquirer.prompt([
          {
            ...
            validate: (input) => {
              if (!input.trim())
                return "Dodatkowe informacje są wymagane dla referencji";
              return true;
            },
          },
        ]);
      ...
      !taskInput.additionalInfo ||
      taskInput.additionalInfo.trim() === ""
    ) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          chalk.gray("   Plik PDF został zapisany w katalogu output.\n")
        );
      } else {
...
async function generateCommand(task, options) {
  ...
  if (!validTasks.includes(task.toLowerCase())) {
    console.log(
      chalk.red(
        `Nieprawidłowy typ dokumentu: ${task}. Użyj: references, cert, lub internship`
      )
    );
    process.exit(1);
  }
...
program
  .name("ngo-docs")
  .description(
    "Generator Dokumentów NGO - Tworzenie referencji, certyfikatów i dokumentów stażowych"
  )
  .version("1.0.0");
...
program
  ...
  .description(
    "Wygeneruj dokument bezpośrednio (task: references, cert, internship)"
  )
